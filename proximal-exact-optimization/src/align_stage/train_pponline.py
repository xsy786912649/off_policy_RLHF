...
from loss import dpo_loss, exact_loss, ppr_dpo_loss, ppr_exact_loss
...
def save_model(args, model, tokenizer, sub_folder):
    ...
    if args.zero_stage == 3:
        ...
        # Save sharded ZeRO-3 checkpoint; DeepSpeed writes zero_to_fp32.py alongside
        model.save_checkpoint(save_dir)
        torch.distributed.barrier()
        # Rank 0 converts shards to a single fp32 state dict and saves HF format
        if args.global_rank == 0:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="for proximal policy optimization"
    )
    parser.add_argument(
        "--label_smooth",
        type=float,
    ...
    parser.add_argument(
        ...
        type=str,
        default="pp-exo-pref",
        help="pp-exo-pref|pp-exo-rw|pp-dpo-pref|pp-dpo-rw"
    )
    parser.add_argument(
        "--max_seq_len",
        type=int,
...
def main():
    ...
    ds_config = get_train_ds_config(offload=args.offload,
                                    ...
                                    tb_path=tb_path,
                                    tb_name=tb_name)
    # Offload only optimizer states to CPU, keeping model params on GPU.
    # This frees ~21GB per GPU for ZeRO-3 so that reference models can co-reside.
    ...
    last_model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path, torch_dtype=DTYPE_MAP[args.dtype])
    ...
    # Move reference models to GPU for inference (no deepspeed wrapper needed).
    # With ZeRO-3 + offload_optimizer the training engine only occupies ~7GB per GPU
    # (param shards + grad shards), leaving enough room for two 14B fp16 reference
    # models (28GB each) to co-reside safely within 80GB.
    ref_model = ref_model.to(device).eval()
    last_model = last_model.to(device).eval()
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                ...
                del ref_outputs
                last_outputs = last_model(**batch)
                last_logits = last_outputs.logits.detach().clone()
                del last_outputs
                torch.cuda.empty_cache()
                
            if "pp-dpo" in args.loss_type:
                loss = ppr_dpo_loss(logits / args.temp, 
                                last_logits / args.temp,
                                ...
                                detla_beta_model=args.detla_beta_model,
                                loss_type=args.loss_type)
            
            elif "pp-exo" in args.loss_type:
                loss = ppr_exact_loss(logits / args.temp, 
                                    last_logits / args.temp,
                                    outputs.logits / args.temp, 
                                    batch["attention_mask"], 
                                    batch["input_ids"], 
