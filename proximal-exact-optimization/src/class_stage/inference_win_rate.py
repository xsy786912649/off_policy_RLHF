...
def parse_args():
    ...
    parser.add_argument("--sft_data_path", type=str, default="")
...
def main():
    ...
    # prepare save path
    if args.mode == "win_rate":
        save_path = os.path.join(args.data_path, "orc_win_rate")
    elif args.mode == "eval":
    ...
    for i in range(len(samples)):
        ...
        if args.mode == "label_rw":
            ...
            instance["win_rate"] = []
            chosen_id = cand_scores
            rejected_id = cand_scores
            if chosen_id == 0:
                mean_rewards += 1
            total_num += 1
    ...
    if args.mode == "eval":
        ...
        print("Win rate: ", mean_rewards / total_num)
        with open(os.path.join(save_path, "win_rate.json"), "w") as f:
            json.dump({"win rate": mean_rewards / total_num}, f, indent=4)
