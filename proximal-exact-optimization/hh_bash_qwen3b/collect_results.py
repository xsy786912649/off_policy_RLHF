"""
Collect all Qwen2.5-3B Anthropic-HH win rate results.
...
INFER_BASE = "exp/hh_qwen3b_exp/data/hh_infer_res/models/qwen2.5-3b_hh"
SFT_EVAL   = f"{INFER_BASE}/sft/eval/test.json"
HH_EVAL    = "exp/hh_qwen3b_exp/data/hh/eval/test.json"
...
def win_rates(sft_data, hh_data, ckpt_data):
    ...
    for s, h, c in zip(sft_data, hh_data, ckpt_data):
        ...
        wh = sum(rc >= max(r_hh) for rc in r_c) / len(r_c)
        win_hh += wh
...
def main():
    ...
    out = "hh_bash_qwen3b/results.json"
